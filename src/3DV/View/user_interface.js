...
export default class UserInteraction {
    constructor(refContainer, scene, renderer, lights, fog, object, animations, minimapCamera, player) {
        ...
        this.gui = new GUI({hideable: false, autoplace: false});
        const guiStyle = {
            position: "absolute"
        }
        const guiDomElement = this.gui.domElement;
        Object.assign(guiDomElement.style, guiStyle);

        refContainer.appendChild(this.gui.domElement);
        ...
        // Create a minimap folder that will have the option to input the minimap zoom
        const minimapFolder = this.gui.addFolder("Minimap");
        minimapFolder.close()
        const minimapInput = {zoom: 0.5};
        minimapFolder.add(minimapInput, "zoom", 0.1, 2.0, 0.1).name("Zoom").onChange(zoom => minimapCamera.setZoom(zoom));
        ...
        const walkingFolder = characterFolder.addFolder("Movement");
        const walkingSpeed = {speed: 1.3};
        walkingFolder.add(walkingSpeed, "speed", 0.1, 5.0, 0.1).name("Speed").onChange(speed => player.setWalkingSpeed(speed));
    }
